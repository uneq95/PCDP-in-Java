...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                .flatMapToPair(integerTuple2Tuple2 -> {
                    Tuple2<Website, Double> websiteRankTuple2 = integerTuple2Tuple2._2();
                    Website website = websiteRankTuple2._1();
                    Double rank = websiteRankTuple2._2();

                    List<Tuple2<Integer, Double>> contributions = new ArrayList<>();

                    Iterator<Integer> it = website.edgeIterator();
                    while (it.hasNext()){
                        contributions.add(new Tuple2<>(it.next(),rank/website.getNEdges()));
                    }
                    return contributions;
                });
        return newRanks.reduceByKey((Double r1, Double r2)->r1+r2).mapValues(value -> 0.15+0.85*value);
    }
}
